# implementation of Spaceship - program template for RiceRocks JGDC
import simplegui
import math
import random
...
def draw(canvas):
    global time, started, lives, score, my_ship, rock_group, explosion_group, missile_group
    # animiate background
    time += 1
    wtime = (time / 4) % WIDTH
...
def group_collide(group, other):
    ...
    for element in group:
        if element.collide(other):
            ...
            chrashed.add(element)
            chrash += 1
    if len(chrashed) > 0:
        group.difference_update(chrashed)
    return chrash
...
def group_group_collide(first_group, second_group):
    ...
    for element in first_group:
        if group_collide(second_group, element) > 0:
            ...
            chrash += 1
    if len (chrashed) > 0:
        first_group.difference_update(chrashed)
    return chrash
